...
class _AddExpensePageState extends State<AddExpensePage> {
  ...
  void _calculatePercentSplit() {
    List<Map<String, dynamic>> selectedMembers = members
    ...
    if (selectedMembers.isNotEmpty) {
      selectedMembers[0]['percentage'] =
          (selectedMembers[0]['percentage'] as int) + remainder;
    }
  }
  ...
  void _calculateAmountSplit() {
    ...
    if (selectedMembers.isNotEmpty) {
      selectedMembers[0]['fixedAmount'] =
          (selectedMembers[0]['fixedAmount'] as int) + remainder;
    }
  }
  ...
  void _showPercentageDialog() {
    ...
    List<Map<String, dynamic>> activeMembers = members
        .where((m) => m['isSelected'] == true)
        .toList();

    if (activeMembers.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Vui lòng chọn người tham gia trước!")),
      );
      return;
    }
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            int totalPercent = activeMembers.fold(
              0,
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    if (totalPercent == 100) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text("Tổng phải bằng 100%")),
                      );
                    }
                  },
  ...
  void _showAmountDialog() {
    ...
    showDialog(
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            int currentTotal = activeMembers.fold(
              0,
              (sum, item) => sum + (item['fixedAmount'] as int),
            );
            ...
            return AlertDialog(
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Text(
                      "Tổng cộng: ${currentTotal}đ",
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        color: currentTotal == inputTotalAmount
                            ? AppColors.primary
                            : Colors.red,
                      ),
                    ),
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    if (currentTotal == inputTotalAmount) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text("Tổng phải khớp với số tiền đã nhập"),
                        ),
                      );
                    }
                  },
        ...
        child: Column(
          ...
          children: [
            ...
            _buildTextField(
              ...
              hint: "Ví dụ: Trả tiền nước tháng 10",
            ),
            const SizedBox(height: 16),
            _buildLabel("Số tiền"),
            ...
            Wrap(
              spacing: 8,
              children: ["Điện nước", "Internet", "Tiền nhà", "Đi chợ"]
                  .map(
                    (c) => ChoiceChip(
                      label: Text(c),
